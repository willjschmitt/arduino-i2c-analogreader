		...
		init: function (options) {
			...
			return this.each(function(){
				...
				var dataType 		= $element.data("type"),
					...
					dataMin		 	= $element.data("min"),
					dataMax		 	= $element.data("max"),
					dataUnit		= $element.data("unit"),
					metaData		= {
						type: 		dataType,
				...
				var center 		= settings.size /2,
					...
					template	=	'<div class="cc-spc-wrapper" role="pie-chart" aria-valuemin="'+settings.min+'" aria-valuemax="'+settings.max+'" aria-valuenow="0" data-valuenow="0" data-diameter="'+settings.size+'" data-min="'+settings.min+'" data-max="'+settings.max+'">' +
										'<div class="cc-spc-chart-percent"><span>'+settings.value+'</span>'+settings.unit+'</div>'+
	...
	var functions = {
		setProgress: function(element,value){
			...
			var duration 	= wrapper.data("duration"),
				...
				percent = pi - ((value-min)*pi/(max-min));
			...
			if (duration) {
				...
				console.log(valTo,valFrom,percent);
			...
			wrapper.attr("aria-valuenow",percent*100);
			wrapper.data("valuenow",percent*100);
		}
	}// end functions
